...
## HermanPrompt Compatibility Note

- The new portal session and `/apps` launcher flow does not require HermanPrompt to change its entrypoint.
- HermanPrompt should continue to receive a signed Prompt launch token and create its own app session from `GET /api/session/bootstrap`.
- Herman Portal login no longer redirects directly into HermanPrompt.
- The new production path is:
  - portal login
  - server-backed portal session
  - `/apps`
  - `POST /api/auth/launch/prompt`
  - redirect into HermanPrompt
- HermanPrompt compatibility depends on the Prompt launch-token contract remaining stable.
- `HERMANPROMPT_LAUNCH_SECRET` in `herman_portal` must exactly match `AUTH_LAUNCH_SECRET` in `herman-prompt`.
- Important contract risk:
  - HermanPrompt currently derives its effective `user_id_hash` from `external_user_id` during launch-token resolution.
  - Herman Portal also includes a `user_id_hash` claim in the launch token.
  - These two systems must not drift.
  - Before broader cross-app auth work continues, the Prompt launch contract should be treated as frozen and should explicitly define whether HermanPrompt must trust the `user_id_hash` claim directly or continue deriving `user_id_hash` from `external_user_id`.
